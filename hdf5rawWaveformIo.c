    ...
    struct waveform_attribute *wavAttr)
{
    ...
    // make a 1-D array template for the yoff, ymult, and yzero waveform attributes.
    // an array is used in case that the ADC is different for each of the SCOPE_NCH
    // number of channels.
    ...
    // insert the waveform attributes defined in common.h into the array. Note that the 
    // type for each matches those in common.h
    ...
    // Waveform Attribute hdf5 attribute is to be attached to the wavefile HDF5 file. This is the same
    // group that the npt, nch, nevents, attributes were attached to.
    rootGid = H5Gopen(wavFile->waveFid, "/", H5P_DEFAULT);
...
int HDF5IO(read_event_int)(struct HDF5IO(waveform_file) *wavFile,
                       struct HDF5IO(waveform_event_int) *wavEvent)
{
    char buf[NAME_BUF_SIZE];
    herr_t ret;
    size_t chunkId, inChunkId;
...
int main(int argc, char **argv)
{
    ...
    struct HDF5IO(waveform_event_int) evt = {
        .eventId = 0,
